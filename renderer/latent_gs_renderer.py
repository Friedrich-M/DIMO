...
class GaussianModel:
    ...
    def __init__(self, sh_degree:int, num_latent_code:int = 1, latent_code_dim:int = 32):
        ...
        latent_codes = torch.randn((self.num_latent_code, self.latent_code_dim), device="cuda")
        self._latent_codes = nn.Parameter(latent_codes.requires_grad_(True))
        # deformation network
    ...
    @property
    def get_latent_codes(self, index=None):
        if index is not None:
            return self._latent_codes[index]
        return self._latent_codes
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._latent_codes], 'lr': training_args.latent_code_lr_init, "name": "latent_code"},    
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if stage >= "s2":
                if param_group["name"] == "c_xyz":
                    ...
                    param_group['lr'] = lr
                elif param_group["name"] == "latent_code":
    ...
    def save_model(self, path, step=None):
        if not step:
            torch.save(self._latent_codes, os.path.join(path, "latent_codes.pth"))
            torch.save(self._timenet.state_dict(), os.path.join(path, "timenet.pth"))
        else:
            torch.save(self._latent_codes, os.path.join(path, "latent_codes_{}.pth".format(step)))
            torch.save(self._timenet.state_dict(), os.path.join(path, "timenet_{}.pth".format(step)))

    def load_model(self, path, step=None):
        print("loading model from exists{}".format(path))
        if not step:
            latent_codes_weight = torch.load(os.path.join(path, "latent_codes.pth"),map_location="cuda")
            weight_dict = torch.load(os.path.join(path, "timenet.pth"),map_location="cuda")
        else:
            latent_codes_weight = torch.load(os.path.join(path, "latent_codes_{}.pth".format(step)),map_location="cuda")
            weight_dict = torch.load(os.path.join(path, "timenet_{}.pth".format(step)),map_location="cuda")
        self._latent_codes = nn.Parameter(latent_codes_weight.requires_grad_(True)) # (num_latent_code, latent_code_dim)
        self._latent_codes = self._latent_codes.to("cuda")
        print(f"{self.num_latent_code} x {self.latent_code_dim} latent codes loaded")
...
class Renderer:
    ...
    def arap_loss(self, t=None, delta_t=0.05, t_samp_num=2, stage="s1", latent_index=0):
        ...
        latent_code = self.gaussians._latent_codes[latent_index] # 32
    ...
    def render(
        ...
        latent_code = self.gaussians._latent_codes[latent_index]
