...
flowchart LR
    image(["🖼️ one reference image"])
    captions("<b>1 · Captions</b><br/>GPT-5 / Qwen3.5")
    videos("<b>2 · Videos</b><br/>Wan2.2 / CogVideoX")
    filter("<b>3 · Filter</b><br/>RAFT + VLM judge")
    multiview("<b>4 · Multi-view</b><br/>SV4D 2.0 / 1.0")
    dataset("<b>5 · Dataset</b><br/>assemble + mattes")
    projector("<b>6 · Projector</b><br/>BERT → latent")
    train(["✨ train.py — 4D Gaussians"])

    image -- "object-centred canvas" --> captions
    captions -- "motions.json" --> videos
    videos -- "one clip per caption" --> filter
    filter -- "clips/ + filter_report.json" --> multiview
    multiview -- "view_XX/FF.png + info.json" --> dataset
    dataset -- "data/&lt;object&gt;" --> train
    dataset -. "captions.json" .-> projector
    train -. "latent codes + motion_order.json" .-> projector

    classDef data fill:#1E1B4B,stroke:#1E1B4B,color:#F8FAFC
    classDef llm fill:#A855F7,stroke:#9333EA,color:#FFFFFF
    classDef video fill:#6D28D9,stroke:#5B21B6,color:#FFFFFF
    classDef vision fill:#0EA5E9,stroke:#0284C7,color:#FFFFFF
    classDef pack fill:#0F766E,stroke:#115E59,color:#FFFFFF
    classDef out fill:#FFD21E,stroke:#CA8A04,color:#1E1B4B

    class image data
    class captions,projector llm
    class videos,multiview video
    class filter vision
    class dataset pack
    class train out
...
Stage 6 is optional and dashed because it runs *after* training: it needs the latent codes the model learned. Everything else is a straight left-to-right chain.
